import React from "react";

function Visit() {
  return (
    <div className="px-[122px] sm:mx-4  sm:px-0 sm:py-[30px] bg-white py-[60px]">
      <div className="py-[45px] bg-[#0077B3] px-[60px] sm:px-4 sm:flex-col flex items-center justify-between">
        <h2 className="text-[#ffffff] font-medium sm:text-center leading-[36px] text-[24px]">
          Do you also want to change your world?
        </h2>
        <h2 className="bg-[#ffffff] text-[#333333] sm:mt-8 sm:text-center text-[24px] font-medium leading-[30px] py-[15px] px-[30px]">
          Visit the VU Master's Event
        </h2>
      </div>
    </div>
  );
}

export default Visit;
